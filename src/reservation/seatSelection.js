...
import { cinemas } from "../data/cinemas.js"; // Cinemas dizisini içeri aktar
...
export function showSeatSelection(cinemaId, salonId, selectedDate, selectedTime) {
  ...
  if (!cinema) {
    console.error("Sinema bilgisi bulunamadı. Gelen cinemaId:", cinemaId);
  ...
  if (!salon) {
    console.error("Salon bilgisi bulunamadı. Gelen salonId:", salonId);
  ...
  if (addToCartButton) {
    addToCartButton.addEventListener("click", () => {
      const details = selectedSeats.map((seat, index) => {
        ...
        return {
          cinema: cinema.name, // Sinema adı buradan alınır
          salon: salon.name,
